create database if not exists CV_bank1;
...
select count(*) from bank_data where age < 30;

select age , job from bank_data where age < 50;

select * from bank_data where (age < 30 or marital = 'single') and balance < 200;

select * from bank_data where balance < 400 and age = 60;

select * from bank_data order by age desc limit 5;

select min(age) from bank_data;

select max(age) from bank_data;

select sum(balance) from bank_data;

select count(*) from bank_data where balance <= 0;

select max(age) from bank_data;

select * from bank_data where age=60;

select age, job from bank_data where age = (select max(age) from bank_data);
...
-- Minimum balance
select min(balance) from bank_data;

-- Maximum balance
select max(balance) from bank_data;

-- The persons who is having loan 
select * from bank_data where loan = 'yes';

-- Find out average balance for all the people whose job role is admin 
select avg(balance) from bank_data where job='admin.';

-- Find out a record  without job whose age is below 45 
select * from bank_data where job='retired' and age < 45;

-- Find out a record where education is primarty and person is jobless
select * from bank_data where education = 'primary' and job = 'retired';

-- Find of a record whose bank account is having a negative balance 
select * from bank_data where balance < 0;

-- Find our a record who is not having house at all along with there balance 
select housing, balance from bank_data where housing = 'no';

use CV_bank1;

load data infile
'C:\Users\TOUCHPIX\OneDrive\Desktop\OPTICAL ARC\task.csv'
into table bank_data fields terminated by ','
